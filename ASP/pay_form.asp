...
function init(type){
    ...
    $('#STPG_payForm [name="custIp"]').val("<%= Request.ServerVariables("REMOTE_ADDR") %>"); //고객 IP 세팅
...
function autoPay(){
    init('mobileAuto');
    $('#STPG_payForm2').attr("action", "pay_autoPayResult.asp");
...
/** pay_receiveResult.asp로부터 응답 값을 받아와서 main폼에 세팅 */
...
<body>
<button onclick="location.href='pay_form.asp'">결제 샘플로</button>
<button onclick="location.href='cancel_form.asp'">취소 샘플로</button>
<button onclick="location.href='encryptTest.asp'">COM+ 컴포넌트 호출 테스트</button>
<div class="wrapper">  
    <form id="STPG_payForm" name="STPG_payForm" >
        ...
        <input type="hidden" name="notiUrl" value="http://localhost/receiveNoti.asp" />         <!-- 결과처리 URL -->
        <input type="hidden" name="nextUrl" value="http://localhost/pay_receiveResult.asp" />   <!-- 결과화면 URL -->
        <input type="hidden" name="cancUrl" value="http://localhost/pay_receiveResult.asp" />   <!-- 결제취소 URL -->
