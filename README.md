...
- **receiveNoti.asp**: 결제 또는 취소 처리가 완료된 후, 헥토파이낸셜가 가맹점으로 전달하는 노티(결과통보)를 수신하는 페이지입니다.
...
- **pay_receiveResult.asp**: 결제창에서 결제가 완료된 이후 닫기 버튼을 누를때, 헥토파이낸셜로 부터 응답파라미터를 수신하는 페이지입니다.
...
- **cancel_showResult.asp**: 헥토파이낸셜와 Server to Server 로 커넥션하여, 취소 요청을 하고 응답을 받아 결과를 출력하는 페이지입니다.
